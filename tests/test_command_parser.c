...
int main(void)
{
    ...
    result = command_process(&state, &callbacks, CMD_PING, NULL, 0);
    ...
    result = command_process(&state, &callbacks, CMD_RESETT, NULL, 0);
    assert(result == CMD_RESULT_OK);
    assert(state.reset_requested == true);

    //result = command_process(&state, 0x99, NULL, 0);
    result = command_process(&state, &callbacks, 0x99, NULL, 0);
